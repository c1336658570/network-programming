/*
    多播的实现
    多播的接收者(./14.12news_receiver 224.1.1.2 9190)
    struct ip_mreq
    {
...
int main(int argc, char *argv[])
{
    ...
    join_adr.imr_multiaddr.s_addr = inet_addr(argv[1]); //写入加入组的IP地址
    join_adr.imr_interface.s_addr = htonl(INADDR_ANY);  //加入该组的套间字所属主机的IP地址，也可以使用INADDR_ANY
